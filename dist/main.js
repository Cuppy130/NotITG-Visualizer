...
let player1 = new THREE.Mesh(new THREE.PlaneGeometry(4, 8), new THREE.MeshBasicMaterial({
    transparent: true,
    opacity: 0.5
}));
let player1Mods = {
...
const receptors = new Array(4).fill(0).map((_, i) => {
    const receptor = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.MeshBasicMaterial({
        transparent: true,
        map: receptorImage[0]
    }));
...
const receptorGlow = new Array(4).fill(0).map((_, i) => {
    const glow = new THREE.Mesh(new THREE.PlaneGeometry(1.5, 1.5), new THREE.MeshBasicMaterial({
        transparent: true,
...
async function loadSong(files) {
    smFile = files?.find(file => file.name.endsWith('.sm'));
    smFile.file((file) => {
    ...
    let luaFolder = files.find(file => file.name === 'lua');
    luaFolder.createReader().readEntries((entries) => {
        const modsLuaFile = entries.find(file => file.name === 'mods.lua');
